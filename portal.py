...
"""
Helper classes to interact with the SciLifeLab Order Portal
(https://cryoem.scilifelab.se), used by the SLL EMhub instance.

Moved here from core (emhub/data/imports/scilifelab.py) on 2026-09-22:
this code is entirely specific to the SLL customization (it talks to the
SciLifeLab Order Portal REST API and imports Portal JSON exports), and
nothing else in core or in any other instance customization ever imported
it. See README.rst, section 5, for the full story.

PortalManager is loaded by app_setup.py (setup_app()) and attached to the
Flask app as ``app.sll_pm`` whenever ``SLL_PORTAL_API`` is present in the
instance's config.py. PortalData is a one-time DB-seeding tool, used only
via this file's own command-line entry point below; it does not run as
part of the normal app.
"""
...
class PortalManager:
    ...
    def _fetchJsonFromUrl(self, url):
        ...
        try:
            ...
            # The Portal is unreachable (no network/VPN, DNS failure, host
            # down, timeout, etc). This is expected in local/dev instances
            # that don't have access to it, so degrade gracefully instead
            # of taking down whatever page triggered this request.
            print("PortalManager: could not reach '%s' (%s)." % (url, e))
            return None
    ...
    def fetchOrdersJson(self):
        """ Fetch orders from the Portal. Returns an empty list (instead
        of raising) if the Portal could not be reached. """
        result = self._fetchJsonFromUrlSuffix('orders?recent=False')
    ...
    def fetchAccountsJson(self):
        """ Retrieve the users list from the portal system. Returns an
        empty list (instead of raising) if the Portal could not be
        reached. """
        result = self._fetchJsonFromUrlSuffix('accounts')
        return result['items'] if result else []
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("\nUSAGE:\n"
              "    python portal.py portal-data.json booked-data.json\n"
              "(run from emhub-sll-cryoem, with the emhub package importable,\n"
              " e.g. the same env used to run the emhub server)\n")
        sys.exit(1)
